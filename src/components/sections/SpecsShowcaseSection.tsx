...
export default function SpecsShowcaseSection() {
  ...
  return (
    <section
      id="specs"
      ref={rootRef}
      className="relative w-full py-28 px-4 sm:px-8 bg-neutral-950 text-white border-t border-neutral-900 overflow-hidden"
    >
      {/* Background Lighting */}
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        {/* Header */}
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-16 border-b border-neutral-800/80 pb-8">
          <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {SPECS.map((group, idx) => (
            <div
              key={idx}
              className="spec-card-anim bg-neutral-900/40 border border-neutral-800/80 rounded-2xl p-6 backdrop-blur-sm hover:border-neutral-700 transition-all shadow-xl"
            >
        ...
        <div className="mt-12 p-6 sm:p-8 rounded-2xl bg-gradient-to-r from-neutral-900/90 via-neutral-900/60 to-neutral-900/90 border border-lime-400/30 flex flex-col lg:flex-row items-center justify-between gap-6 shadow-[0_0_40px_rgba(163,230,53,0.08)]">
          <div className="flex items-center gap-5">
            <div className="w-12 h-12 rounded-xl bg-lime-400/10 border border-lime-400/40 flex items-center justify-center text-lime-400">
              <Gauge className="w-6 h-6" />
            </div>
            <div>
              <div className="font-mono text-sm font-bold text-white">
                WORLD ATHLETICS COMPLIANT (2026 GUIDELINES)
              </div>
              <div className="font-mono text-xs text-neutral-400 mt-0.5">
                Maximum 38mm stack height and single rigid carbon plate certified for official competition and major road marathons.
              </div>
            </div>
          </div>
